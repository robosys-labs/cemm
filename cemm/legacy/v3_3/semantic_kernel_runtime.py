...
from ...kernel.model.cycle import KernelSnapshot, pin_snapshot
...
class SemanticKernelRuntime:
    ...
    def __init__(
        ...
        from ...kernel.understanding.percept_bridge import PerceptToSurfaceEvidence
        from ...kernel.understanding.composer import SemanticComposer
        from ...kernel.understanding.grounding import GroundingResolver
        from ...kernel.understanding.interpreter import InterpretationResolver as V34InterpretationResolver
        from ...kernel.understanding.gap_detector import GapDetector as V34GapDetector
        ...
        from ...kernel.execution.planner import Planner as V34Planner
    ...
    def run_turn(
        ...
        *,
        ...
        """Hybrid v3.4 migration cognitive cycle.
                ...
                for cand_prop in candidate_graph.candidate_propositions:
                    ...
                    for cc in candidate_graph.candidate_contexts:
                        if cc.context_frame.id == prop.context_ref:
                            ...
                            break
                    if ctx is None:
                        from ...kernel.model.context_frame import ContextFrame
        ...
        try:
            from ...kernel.self_model.capability_evaluator import CompetenceRecord, ImplementationRecord
        ...
        try:
            if plan_batch and plan_batch.selected:
                from ...kernel.execution.authorizer import AuthorizationConditions, AuthorizationStatus, AuthorizationBatch
                cap_status = getattr(capability_assessment, "status", "incapable") if capability_assessment else "incapable"
                cap_available = cap_status in ("capable", "degraded")
                auth_by_op: dict[str, Any] = {}
        ...
        commit_outcome = None
        try:
            from ...kernel.model.mutation import MutationSet, MutationOperation
        ...
        message_plan = None
        try:
            from ...kernel.response.planner import ContentSelectionInput
            from ...kernel.model.epistemic import EpistemicAssessment
            prop_refs = tuple(
        ...
        # BF-005: If v3.4 CommitCoordinator exists and required_satisfied is
        # False, legacy write success claims must be overridden. There can be
        # only one mutation authority. Until PatchCommitter is retired, do not
        # let legacy commit results claim success when v3.4 commit failed.
        if commit_outcome is not None and not commit_outcome.required_satisfied:
            if write_outcome.commit_status == "committed":
                write_outcome = WriteOutcome(
                    patch_count=write_outcome.patch_count,
                    committed_count=0,
                    rejected_count=write_outcome.committed_count,
                    quarantined_count=write_outcome.quarantined_count,
                    commit_status="rejected",
                    committed_record_ids=[],
                    rejected_patch_ids=write_outcome.committed_target_ids,
                    rejected_reasons=["v3.4 CommitCoordinator did not satisfy required mutations"],
                    required_target_ids=write_outcome.required_target_ids,
                    committed_target_ids=[],
                    operation_results={
                        t: "rejected" for t in write_outcome.operation_results
                    },
                )
        ...
        try:
            if message_plan and result.realized_output:
                from ...kernel.response.common_ground import (
        ...
        diag["pipeline"] = "hybrid_v34_migration"
        diag["v34_authoritative"] = False
        diag["legacy_authorities"] = [
            "SemanticCPU", "SemanticQueryEngine", "PatchCommitter",
            "ResponseFormationEngine", "OperationalContractCompiler",
        ]
        if diag:
    ...
    def _build_write_outcome(
        ...
        # BF-004: Removed false write-success fallback that inferred required
        # targets from arbitrary patch operations when no explicit required
        # targets existed. An auxiliary concept/schema write cannot satisfy
        # a missing relation write.

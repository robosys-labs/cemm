...
```text
input: semantic_latent_target example
expect: positive/negative pairs cite typed semantic structure; no target asks embedding to hide permission, source, time validity, or confidence
```

Tool handoff planning:

```text
input: "What is 17.5% of 420000?"
expect: tool_handoff_planning selects calculator/basic-style tool schema; exact computation is represented as structured tool input, not approximated prose
```

Procedure model induction:

```text
input: repeated virtual meeting scheduling traces
expect: candidate ProcedureModel with required slots and tool sequence; candidate remains inactive until validation
```

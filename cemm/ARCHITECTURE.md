...
Version: 3.1  
Status: implementation reference (all phases 0-9 complete)  
Audience: medium to senior programmers implementing CEMM without architectural drift  
Scope: semantic compression, construction grammar, operational ports, dynamic concept grounding, unsupervised learning, and budget-aware response formation

## 1. Purpose

This document consolidates the architecture across all CEMM subsystems:

```text
semantic_compression_and_unsupervised_learning.md   [archived]
construction_grammar_for_cemm.md                     [archived]
operational_ports_and_dynamic_slot_resolution.md     [archived]
atom_grounding_and_bootstrapping.md                  [archived]
dynamic_affordances_and_effects.md                   [archived]
concept_lattice_runtime_resolution.md                [archived]
cemm-v3.1-lean-implementation-plan.md                [complete, response formation]
semantic-schema-refactor.md                           [complete, schema kernel]
causal-runtime-wiring-fix.md                          [partially addressed]
```
...
- `AGENTS.md` — governing implementation guide for AI coding agents
- `cemm-v3.1-lean-implementation-plan.md` — response formation architecture plan (all phases complete)
- `semantic-schema-refactor.md` — Semantic Schema Kernel refactor record (complete)
- `causal-runtime-wiring-fix.md` — causal-runtime wiring fix plan (partially addressed)
- `3.3-uol-graph-architecture.md` — UOL graph architecture details
- `core_loop_runtime.md` — core loop runtime contract (superseded by this document for pipeline details)
...
-> TeachingFrameManager (teaching frame extraction)
...
-> ResponseFormationEngine:
     ...
     -> RealizationExecutor (language-specific rendering via LanguageRenderer)
...
### 5.0 Semantic Schema Kernel (Implemented)
...
```text
EntityKindRegistry        — entity kinds with native slot families
...
### 5.0a Response Formation (v3.1 — Implemented)

The canonical response path replaces the retired `SemanticRealizer`:
...
-> PlanGateAndRanker          (language-agnostic: hard gate validation + plan scoring and ranking)
-> Selector                   (selects best candidate per budget)
...
- `PrimitiveGoalComposer` and `ResponseMoveComposer` are language-agnostic — no English surface string classification
- `RealizationExecutor` delegates to `LanguageRenderer` modules — no English wording in the executor
- `SlotBinder` binds from evidence only — never reads raw user text
- `RealizationUnit` is the language-neutral IR keeping language-specific rules out of response planning
- `WriteOutcome.committed` must be checked before claiming memory was stored
- Safety goals are gates, not ranker preferences
- No fallback to retired `SemanticRealizer` — errors must be visible
- `RealizationContract` and `template_key` are RETIRED — no backward compatibility layers exist
- Pronoun handling is built into language renderer templates, not regex-based shifting
- Budget reduces exploration, never safety gates
- Rejected candidates remain diagnosable in `ResponseBundle.diagnostics`

### 5.0b Budget-Aware Query (Phase 6 — Implemented)

`BudgetAwareSemanticQueryEngine` wraps `SemanticQueryEngine` with budget constraints:

```text
BudgetDecision -> query time budget -> build_query -> execute -> AnswerBinding
```

### 5.0c Internal Actions (Phase 8 — Implemented)

`InternalActionProposer` proposes actions from response moves. `InternalActionAuthorizer` gates them:

```text
ResponseMoves -> InternalActionProposer -> proposed actions -> InternalActionAuthorizer -> authorized/rejected
```

### 5.0d Response/Budget Learning (Phase 9 — Implemented)

`ResponseBudgetLearningExtractor` extracts learning patches from response outcomes:

```text
ResponseBundle + BudgetDecision -> learning observation -> training export
```

### 5.0e Deliberation (Phase 7 — Implemented)

`DeliberationPlanner` and `AnytimeDistiller` provide deliberation planning:

```text
BudgetDecision -> deliberation depth -> distill intermediate results
```
...
```text
cemm/types/
  uol_graph.py, graph_patch.py, meaning_percept.py, runtime_cycle.py, turn_trace.py
  semantic_answer_graph.py, packets.py, meaning_percept.py, etc.
  (Foundational atoms + runtime packets)

cemm/kernel/
  semantic_kernel_runtime.py    — single authoritative runtime orchestrator
  semantic_cpu.py               — operator orchestration
  semantic_attention_controller.py — focus selection
  semantic_program_compiler.py  — compile semantic programs
  semantic_obligation_scheduler.py — schedule obligations
  semantic_query_engine.py      — build + execute semantic queries
  teaching_frame_manager.py     — teaching frame extraction
  relation_frame_compiler.py    — compile relation frames with projection policies
  relation_algebra.py           — relation algebra for query execution
  meaning_perceptor.py          — signal -> meaning groups and initial atoms
  meaning_graph_builder.py      — meaning packet -> working UOL graph
  construction_matcher.py       — surface/graph patterns -> construction candidates
  act_resolution_planner.py     — graph/operator state -> reply/action/retrieval/memory plans
  safety_frame_detector.py     — safety frame detection
  output_state_updater.py       — output state update
  session_store.py             — session persistence
  entity_salience_tracker.py   — entity salience tracking
  anaphora_resolver.py         — anaphora resolution
  predicate_phrase_extractor.py — predicate phrase extraction
  implicit_predicate_detector.py — implicit predicate detection
  semantic_schema_kernel.py    — Semantic Schema Kernel (7 registries)
  schema_backed_language_adapter.py — schema-driven language adapter
  pipeline.py                  — Pipeline + PipelineResult
  training_tasks.py            — training task specifications
  training_export.py           — training data export
...
cemm/response/
  ...
  types.py                     — ResponseBundle, ResponseSituation, BudgetFrame, etc.
  realization/
    ...
    languages/
      ...
      fr.py                    — French renderer
  transformers/
    candidate_generator.py     — generates candidate plans with framing variants
    framing_variant.py         — language-agnostic framing variants
...
cemm/actions/
  proposer.py                  — InternalActionProposer
  authorizer.py                — InternalActionAuthorizer
  types.py                     — action types

cemm/deliberation/
  deliberation_planner.py      — DeliberationPlanner
  anytime_distiller.py         — AnytimeDistiller

cemm/query/
  budget_aware_semantic_query.py — BudgetAwareSemanticQueryEngine

cemm/learning/
  patch_validator.py           — graph patch validation
  patch_committer.py           — graph patch commitment
  observation_builder.py       — learning observation builder
  outcome_interpreter.py       — outcome interpretation

cemm/memory/
  concept_lattice.py           — concept lattice (in-memory)
  construction_lattice.py      — construction storage and matching
  durable_semantic_store.py    — durable semantic store (SQLite-backed)
  predicate_schema_store.py    — predicate schema store

cemm/confidence/
  log_odds.py, scoring.py      — confidence math

cemm/causal/
  causal_bridge.py             — causal inference bridge

cemm/data/
  semantic_schemas/             — 7 canonical schema JSON files
  languages/                    — language alias packs
```
...
+ Semantic Schema Kernel (7 registries, schema-driven meaning)
...
+ budget-aware response formation (candidate generation, gating, ranking, selection)
+ language-specific rendering (English, French)
+ internal action proposal and authorization
+ response/budget learning extraction
```
...
## 23. Retired Components (v3.1)

The following components have been removed from the codebase. No backward compatibility layers exist.

```text
SemanticRealizer                    — replaced by ResponseFormationEngine
response_templates.json             — replaced by language renderer templates
RealizationContract type            — replaced by ResponseSituation + ResponseBundle
SemanticQueryEngine.build_contract() — removed
_template_for_obligation()          — removed (template selection now in response engine)
_shift_pronouns_for_echo()          — removed (pronoun handling in language renderers)
_sanitize_echo()                    — removed (sanitization in SlotBinder._clean_value)
_strip_framing_prefix()             — removed
_strip_echo_discourse_markers()     — removed
template_key field                  — retired
realization_contract field          — retired from RuntimeCycleResult + PipelineResult
semantic_realized_output field      — retired from PipelineResult
```

Do not re-introduce these components. If response text is needed, use `ResponseFormationEngine`.

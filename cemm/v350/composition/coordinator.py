...
class MeaningComposer:
    ...
    Selection has no dependency on target-language realization.  A hypothesis
    can win only through evidence, semantic compatibility, discourse/world
    factors compiled into the graph, and bounded complexity tie-breaking.
    """
    ...
    def compose(
        ...
        *,
        ...
        factor_graph = self.builder.build(
            lattice, grounding, context_ref=context_ref, snapshot=snapshot
        )
        solved = self.solver.solve(factor_graph)
        ...
        for hypothesis in solved.hypotheses:
            try:
                uol, report = self.materializer.materialize(
                    ...
                    context_ref=context_ref, snapshot=snapshot,
                )
        ...
        if selected_hypothesis is None:
            ...
            margin = selected_hypothesis.score - next_score if next_score is not None else float("inf")
            close = tuple(
                hypothesis.hypothesis_ref
                for hypothesis, _uol, _report in materialized[1:]
                if selected_hypothesis.score - hypothesis.score <= self.close_alternative_margin
            )
            uncertainty_list = []
            if selected_hypothesis.unresolved_refs or (selected_uol and selected_uol.unresolved_refs):
                uncertainty_list.append("selected meaning retains unresolved semantic frontiers")
            if close:
                uncertainty_list.append("close compatible alternatives are preserved")
            if solved.exhausted:
                uncertainty_list.append("bounded search budget exhausted before full enumeration")
            if selected_report and selected_report.unresolved:
                uncertainty_list.append("UOL validation retains unresolved dependencies")
        ...
        selection = SelectionAssessment(
            selected_hypothesis_ref=None if selected_hypothesis is None else selected_hypothesis.hypothesis_ref,
            decisive=decisive,
            margin=margin,
        ...
        if selected_uol is not None:
            ...
            selected_unresolved = tuple(sorted(
                set(selected_unresolved) | set(selected_uol.unresolved_refs)
            ))
        partial = PartialUnderstandingMap(
            understood_refs=understood,
            unresolved_refs=selected_unresolved,
            frontier_refs=tuple(sorted(set(grounding.frontier_refs) | set(grounding.unresolved_mention_refs))),
            evidence_refs=factor_graph.evidence_refs,
        )
        alternatives = tuple(
            hypothesis for hypothesis, _uol, _report in materialized
            if selected_hypothesis is None or hypothesis.hypothesis_ref != selected_hypothesis.hypothesis_ref
        )
        bundle = MeaningBundle(
            bundle_ref="meaning-bundle:" + semantic_fingerprint(
                "meaning-bundle-ref",
                (
                    factor_graph.graph_ref,
                    None if selected_hypothesis is None else selected_hypothesis.hypothesis_ref,

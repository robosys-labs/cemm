"""Cycle-local workspace, frontier effects, and honest completion semantics."""
...
def infer_frontier_effects(
    ...
    *,
    ...
    if (
        "operation_outcome_unknown" in klass
        or ref.startswith(("frontier:operation-", "frontier:operation:"))
    ):
        effects.add(FrontierEffect.BLOCKS_EFFECT)
    if (
        "realization_gap" in klass
        or ref.startswith(("frontier:realization:", "frontier:response:"))
    ):
        effects.add(FrontierEffect.BLOCKS_REALIZATION)
    if ref.startswith(
        ("frontier:emission:", "frontier:emission-", "frontier:output-discourse:")
    ):
        effects.add(FrontierEffect.BLOCKS_EMISSION)
    if ref.startswith(
        ("frontier:query", "frontier:stage:10", "runtime-frontier:query")
    ):
        effects.add(FrontierEffect.BLOCKS_QUERY_ANSWER)
...
class CycleWorkspace:
    ...
    frontier_effects: dict[str, frozenset[FrontierEffect]] = field(
    ...
    def register_frontier(
        self,
        frontier_ref: str,
        effects: Iterable[FrontierEffect] | None = None,
    ) -> None:
        resolved = (
            frozenset(effects)
            if effects is not None
            else infer_frontier_effects(frontier_ref)
        )
    ...
    def register_runtime_frontiers(self, frontiers: Iterable[Any]) -> None:
        for frontier in frontiers:
            ...
            if explicit:
                effects = tuple(
                    item
                    if isinstance(item, FrontierEffect)
                    else FrontierEffect(str(item))
                    for item in explicit
                )
            else:
                effects = infer_frontier_effects(
                    ref,
                    frontier_class=str(
                        getattr(
                            getattr(frontier, "frontier_class", None),
                            "value",
                            getattr(frontier, "frontier_class", ""),
                        )
                    ),
...
class CompletionEvaluator:
    def evaluate(self, cycle: Any) -> CycleCompletionStatus:
        ...
        for frontier in artifacts.get("runtime_frontiers", ()):
            effects.update(
        ...
        for item in tuple(artifacts.get("operation_outcomes", ())):
            if not isinstance(item, tuple) or len(item) < 5:
                continue
            journal, result = item[3], item[4]
            status = str(
                getattr(
                    getattr(journal, "status", None),
                    "value",
                    getattr(journal, "status", ""),
                )
            )
            result_status = str(
                getattr(
            ...
            if "unknown" in status or result_status == "unknown":
                return CycleCompletionStatus.ACTION_UNCERTAIN

        retrieval = artifacts.get("retrieval_result")
        response_requested = bool(
            getattr(
                getattr(retrieval, "request", None),
                "response_requested",
                False,
            )
        )

        if artifacts.get("emission") is not None:
            if FrontierEffect.BLOCKS_EMISSION in effects:
                return CycleCompletionStatus.RUNTIME_ERROR
            if (
                response_requested
                and FrontierEffect.BLOCKS_QUERY_ANSWER in effects
            ):
        ...
        response_exists = any(
            artifacts.get(key) is not None
            for key in ("response_uol", "surface_candidate", "goal_decision")
        )
        if FrontierEffect.BLOCKS_EMISSION in effects and response_exists:
        ...
        if response_requested and (
            FrontierEffect.BLOCKS_QUERY_ANSWER in effects
            or artifacts.get("surface_candidate") is None
        ):
            return CycleCompletionStatus.RESPONSE_DEFERRED

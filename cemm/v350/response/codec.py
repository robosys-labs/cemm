...
def _selector(v):
 d=dict(v); return ResponseBindingSelector(str(d['output_port_ref']),ResponseSelectorMode(str(d['mode'])),d.get('source_field'),d.get('source_port_ref'),d.get('fixed_ref'),None if d.get('target_index') is None else int(d['target_index']))

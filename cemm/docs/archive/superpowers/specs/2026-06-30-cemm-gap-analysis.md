...
- SemanticEventGraph never populated with temporal/causal/claim/model edges — now populated for causal/temporal/actionable inputs; multi-word entity inference added; model refs match causal input semantics; learned NER tagger (structured perceptron with Viterbi decoding) trains on boltuix/conll2025-ner real dataset with progress output and validation, supports full 36-tag BIO taxonomy mapped to CEMM roles, and extracts real-world entities ✗ (still limited to English; accuracy ~74% token-level on conll2025-ner; no transformer contextual embeddings)
- No typed latent spaces exist — runtime types and encoder added; answer_latent populated; full TypedLatents snapshot captured in Trace; snapshots persist in action store; Ranker uses latent similarity to boost models used in similar successful contexts ✗ (full CEMM-SLC weight learning / gradient-based updates remain)
- Causal inference never fires (empty `causal_edges`) — now fires and produces predictions from seeded, auto-promoted learned, and narrative-discovered causal models ✗ (still limited to explicit causal language and repeated observations; no deep causal reasoning)
- Simulation never runs (empty `causal_edges`) — now runs on causal inputs ✗ (still limited to observed patterns)

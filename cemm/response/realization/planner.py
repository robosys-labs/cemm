...
class RealizationPlanner:
    def build_plan(
        ...
        *,
        ...
        units = [
            unit for move in moves
            if (unit := self._unit_for_move(move, situation, slots, candidate_plan.style)) is not None
        ]
        return RealizationPlan(
            ...
            diagnostics={"unit_count": len(units), "unit_types": [unit.unit_type for unit in units]},
        )
    ...
    def _unit_for_move(
        ...
        if move.move_type in {
            "social_greet", "social_farewell", "phatic_response", "repair_prior_response",
            "clarify", "deescalate", "set_expectation",
        }:
        ...
        if move.move_type == "answer":
            ...
            common = dict(
                ...
                object_values=list(answer.values),
                style=style,
            ...
            if obligation_kind == "answer_self_identity":
                return RealizationUnit(unit_type="self_identity_assertion", subject_role="self", **common)
            if obligation_kind == "answer_self_capability":
                return RealizationUnit(unit_type="capability_assertion", subject_role="self", **common)
            return RealizationUnit(unit_type="generic_assertion", **common)
        if move.move_type == "evidence_explanation":
            path = self._longest_evidence_path(situation)
            return RealizationUnit(
                unit_type="evidence_explanation", move_type=move.move_type,
                evidence_path=path, style=style,
            ) if path else None
        ...
        if move.move_type == "confirm_memory_write":
            answer = slots.get("answer")
            committed = bool(
                situation.write_outcome
                and getattr(situation.write_outcome, "satisfied", situation.write_outcome.committed)
            )
            return RealizationUnit(
                unit_type="memory_confirmation",
                move_type=move.move_type,
                object_value=answer.value if answer else "",
                object_values=list(answer.values) if answer else [],
        ...
        if move.move_type == "safety_refusal":
            return RealizationUnit(
                ...
                move_type=move.move_type,
                safety_category=str(getattr(situation.safety_frame, "category", "") or ""),
                safety_severity=str(getattr(situation.safety_frame, "severity", "") or ""),
                style=style,
            )
        return None
    ...
    def _abstain_unit(situation: ResponseSituation, style: dict[str, float]) -> RealizationUnit:
        ...
        return RealizationUnit(
            unit_type="honest_abstain", move_type="honest_abstain",
            abstention_reason=reason, style=style,
        )

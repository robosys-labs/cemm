"""Small stdlib web demo for the canonical CEMM v3.5 runtime."""
...
def handle_chat(runtime:Runtime,payload:dict[str,Any])->dict[str,Any]:
    ...
    response={'ok':True,'output_text':result.output_text,'cycle_id':result.cycle_id,'context_id':result.context_id,'target_language':result.target_language,'committed_patch_refs':list(result.committed_patch_refs),'emission_authorized':result.emitted,'frontier_refs':list(result.frontier_refs),'errors':list(result.errors)}
...
class DemoHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path=='/health':return self._send_json({'ok':True,'version':Runtime.VERSION})
...
def serve(host='127.0.0.1',port=8765,database_path=':memory:'):
    runtime=Runtime(database_path=database_path);DemoHandler.runtime=runtime;server=ThreadingHTTPServer((host,port),DemoHandler)
    try:print(f'CEMM v3.5 web demo listening at http://{host}:{port}');server.serve_forever()
    finally:server.server_close();runtime.close()

def main():
    p=argparse.ArgumentParser(description='CEMM v3.5 web demo');p.add_argument('--host',default='127.0.0.1');p.add_argument('--port',type=int,default=8765);p.add_argument('--database',default=':memory:');a=p.parse_args();serve(a.host,a.port,a.database)

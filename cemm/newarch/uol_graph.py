"""Universal Ontological Language working graph primitives.

UOLGraph is CEMM's hot-path semantic workbench. It is the temporary graph used
to interpret one signal, turn, transcript window, or candidate interpretation.
It is not the permanent ontology and it is not the durable memory format.

Durable learning must leave this graph through GraphPatch candidates.
"""
...
@dataclass
class UOLAtom:
    """A primitive working-graph node.

    `kind` is one of the fixed kernel atom categories. `key` is dynamic and may
    point at a learned concept lattice key. Domain concepts such as president,
    country, person, weather, or cold belong in `key`, never in `kind`.
    """
...
@dataclass
class UOLEdge:
    """A typed structural relation between working-graph atoms."""
...
class UOLMeaningGroup:
    """Graph-local semantic unit.

    This mirrors the perceptor's meaning groups but is stored with graph IDs so
    later stages can train on segmentation and group-scoped graph structure.
    """
...
@dataclass
class CandidateSet:
    """Competing atom or graph interpretations for a span."""
...
class ConstructionMatch:
    """A form-meaning operator match against surface or graph structure."""
...
class ConceptResolution:
    """Runtime link from a UOL atom to a dynamic concept candidate."""
...
@dataclass
class PortBinding:
    """A dynamic operational-port binding candidate."""
...
class AffordancePrediction:
    """Tentative effect prediction over a bound graph pattern."""
...
@dataclass
class UOLGraph:
    """A temporary UOL graph for one interpretation window."""
    ...
    def clone(self, *, graph_id: str | None = None) -> "UOLGraph":
        """Return a deep copy for parallel hypothesis evaluation."""
    ...
    def merge_graph(self, other: "UOLGraph", *, confidence_floor: float = 0.0) -> None:
        """Merge another working graph into this graph.

        This is intentionally conservative: atoms keep their IDs, duplicate
        atoms/edges are merged through the normal add paths, and low-confidence
    ...
    def prune(self, *, atom_threshold: float = 0.0, edge_threshold: float = 0.0) -> None:
        """Trim low-confidence branches from the working graph."""
        if atom_threshold > 0:
            keep_atom_ids = {
    ...
    def select_candidate(
        ...
        *,
        ...
        """Mark selected/rejected candidates without deleting graph evidence."""
    ...
    def to_training_example(self) -> dict[str, Any]:
        """Return a stable graph-supervision record."""
        return {
            "graph_id": self.id,

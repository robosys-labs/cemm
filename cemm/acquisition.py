...
semantic identity has an explicit reviewer-supplied kind. It never guesses
``concept`` and never retries parsing from inside the parser.
"""
...
def _validated_kind(value: Any) -> str:
    ...
    if not kind:
        raise ValueError(
            "reviewed acquisition requires an explicit semantic kind for each new mention"
        )
    if kind in _FORBIDDEN_NEW_KINDS:
        raise ValueError(
            f"reviewed lexical acquisition cannot create ABI/state-space kind: {kind}"
        )
    return kind


def acquire_reviewed(
    store: Store, runtime: Runtime, document: dict[str, Any]
) -> dict[str, Any]:
    """Publish reviewed identities/designations, then optionally process text."""
    language = str(document.get("language") or runtime.lang)
    document_ref = str(
        document.get("document_ref") or stable("reviewed-document", document)
    )
    ...
    with store.db:
        ...
        for index, mention in enumerate(mentions):
            ...
            existing = store.atom(ref) if ref else None
            if existing:
                if supplied_kind and str(existing["kind"]) != str(supplied_kind):
                    raise ValueError(
                        f"mention {surface!r} kind conflicts with existing identity {ref}: "
                        f"{supplied_kind} != {existing['kind']}"
                    )
            else:
            ...
            label_type = str(mention.get("label_type") or "label:lexical")
            label_atom = store.atom(label_type)
            if not label_atom or str(label_atom["kind"]) != "label_type":
                raise ValueError(
                    f"reviewed acquisition requires an existing label_type: {label_type}"
                )
            args = {
                "role:target": ref,
                "role:label_type": label_type,
                ...
                "role:preferred": lit(
                    bool(mention.get("preferred", True)), "bool"
                ),
            }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Explicit reviewed CEMM lexical acquisition"
    )
    ...
    store = Store(args.db)
    if args.data and not store.db.execute("SELECT 1 FROM atoms LIMIT 1").fetchone():
        store.import_bundle(args.data)
    runtime = Runtime(store, args.pack)
    document = json.loads(Path(args.document).read_text(encoding="utf-8"))
    print(
        json.dumps(
            acquire_reviewed(store, runtime, document),
            ensure_ascii=False,
            indent=2,
        )
    )

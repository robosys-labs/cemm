...
from cemm_authoritative_hybrid.r4_partition_verify import (
...
__cemm_test_inventory__ = {
    'tests/test_r4_partition_global_assignment.py::test_real_graph_reconstructs_all_axes_without_coarse_language_collapse': {'activation_phase': 'R4', 'assertion_ref': 'assertion:r4-global-graph-reconstructs-seven-exact-axes', 'diagnostic_role': 'owner', 'introduced_by_task': 'R4-Partition-Corrective-Task-4', 'owner_ref': 'mutation-partition', 'source_ast_sha256': 'dec8eb0a750ead763e86a89900ec0b0d23e1889fbdb6401ae5a23e69f241894f'},
    ...
    'tests/test_r4_partition_global_assignment.py::test_every_episode_appears_in_exactly_one_class': {'activation_phase': 'R4', 'assertion_ref': 'assertion:partition-leakage-every-episode-apars-in-exactly-one-partition', 'diagnostic_role': 'owner', 'introduced_by_task': 'R4-Partition-Corrective-Task-7', 'owner_ref': 'mutation-partition', 'source_ast_sha256': '0c4145662767635f61c0c7947987bda475daf08df945c0342611d0a23f59fa64', 'supersedes_node_id': 'tests/test_partition_leakage.py::test_every_episode_apars_in_exactly_one_partition'},
    'tests/test_r4_partition_global_assignment.py::test_class_sizes_satisfy_reviewed_integer_objective': {'activation_phase': 'R4', 'assertion_ref': 'assertion:partition-leakage-partition-ratios-are-approximately-balanced', 'diagnostic_role': 'owner', 'introduced_by_task': 'R4-Partition-Corrective-Task-7', 'owner_ref': 'mutation-partition', 'source_ast_sha256': '48b52e360cc60e2cda5f52bd5217ca6dbe6b150df78aed42e03fdd650a3c9fe1', 'supersedes_node_id': 'tests/test_partition_leakage.py::test_partition_ratios_are_approximately_balanced'},
}
...
def _real():
    episodes = tuple(
        ...
        for line in (ROOT / "artifacts/r4/episodes.jsonl")
        .read_text("utf-8")
        .splitlines()
    )
    mutations = tuple(
        SemanticMutation.from_dict(json.loads(line))
        for line in (ROOT / "artifacts/r4/mutations.jsonl")
        .read_text("utf-8")
        .splitlines()
    )
    config = R4PartitionConfig.from_json_bytes(
        (ROOT / "configs/r4_partitions.json").read_bytes()
    )
...
def assigned():
    ...
    result = GlobalLeakagePartitioner().assign(
        episodes,
        config=config,
        mutations=mutations,
    )
...
def in_memory_manifest(assigned):
    ...
    for split in SPLITS:
        components = tuple(
            row for row in result.evidence.components if row.split == split
        )
        member_refs = tuple(
            sorted(member for row in components for member in row.member_refs)
        )
        raw = b"".join(
            canonical_json_bytes(episode_by_ref[ref].as_dict())
        ...
        classes.append(
            SplitClassRecord.create(
                ...
                component_refs=tuple(
                    sorted(row.component_ref for row in components)
                ),
                label_counts=labels,
            )
        )
...
def test_real_graph_reconstructs_all_axes_without_coarse_language_collapse() -> None:
    ...
    graph = GlobalLeakagePartitioner().build_graph(
        episodes,
        mutations=mutations,
    )
    ...
    assert not any(
        row.key_namespace == "language" and row.key_ref == "en"
        for row in graph.hyperedges
    )
    assert normalized_surface_key("Hello", "en") == normalized_surface_key(
        " hello ",
        "en",
    )
    assert normalized_surface_key("hello", "en") != normalized_surface_key(
...
def test_real_allocator_is_whole_component_deterministic_and_independently_verified() -> None:
    ...
    assert (
        len(
            {
                member
                for row in first.evidence.components
...
def test_independent_verifier_rejects_split_tamper() -> None:
    ...
    components = tuple(
        sorted(
            (replacement, *result.evidence.components[1:]),
...
def test_no_leakage_hyperedge_crosses_classes(
    ...
    edges = tuple(
        row for row in result.evidence.hyperedges if row.axis == axis
    )
    assert lineage_case
    assert edges
    for edge in edges:
...
def test_split_manifest_counts_match_assignment(
    assigned,
    in_memory_manifest,
) -> None:
...
def test_split_payload_counts_match_manifest(in_memory_manifest) -> None:
    ...
    for split in SPLITS:
        row_count = len(
            [line for line in payloads[split].splitlines() if line]
        )
...
def test_split_payload_hashes_match_manifest(in_memory_manifest) -> None:
    ...
    for split in SPLITS:
        assert (
            hashlib.sha256(payloads[split]).hexdigest()
            == classes[split].payload_sha256
        )


def test_every_episode_appears_in_exactly_one_class(
    assigned,
    in_memory_manifest,
) -> None:
...
def test_class_sizes_satisfy_reviewed_integer_objective(
    assigned,
    in_memory_manifest,
) -> None:
    episodes, _, config, result = assigned
    ...
    size_deviation = sum(
        abs(
            RATIO_DENOMINATOR * counts[split]
            - len(episodes) * weights[split]
        )
        for split in SPLITS
    )
    assert tuple(
        (row.split, row.weight)
        for row in config.target_weights
    ) == (
        ("train", 60),
        ("selection", 15),
        ("calibration", 15),
...
def test_synthetic_giant_component_fails_four_class_feasibility() -> None:
    graph = GlobalLeakagePartitioner().build_graph(
        tuple(_synthetic(i) for i in range(8))
    )
    assert len(graph.components) == 1
    _, result = GlobalLeakagePartitioner().analyze_feasibility(
        tuple(_synthetic(i) for i in range(8))
    )

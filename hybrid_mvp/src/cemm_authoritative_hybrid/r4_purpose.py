...
class PurposeMembership:
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> "PurposeMembership":
        ...
        if row["abi_version"] != PURPOSE_CONTRACT_ABI_VERSION:
            raise ValueError("unsupported Purpose Contract ABI")
...
class DuplicateRiskGroup:
    ...
    def create(cls, *, group_ref: str, namespace: str, member_case_refs: tuple[str, ...], reason_ref: str, review_refs: tuple[str, ...]) -> "DuplicateRiskGroup":
        ...
        members = exact_ref_tuple(member_case_refs, "member_case_refs", nonempty=True, maximum=MAX_GROUP_MEMBERS, prefix="expanded_case_v2:")
        if len(members) < 2:
...
class ChallengeHoldout:
    ...
    def create(cls, *, holdout_ref: str, identity_namespace: str, identity_ref: str, purpose: str, member_case_refs: tuple[str, ...], reason_ref: str, review_refs: tuple[str, ...]) -> "ChallengeHoldout":
        ...
        return construct(cls, abi_version=PURPOSE_CONTRACT_ABI_VERSION, holdout_ref=exact_ref(holdout_ref, "holdout_ref", prefix="challenge_holdout:"), identity_namespace=namespace, identity_ref=exact_ref(identity_ref, "identity_ref"), purpose=_purpose(purpose), member_case_refs=exact_ref_tuple(member_case_refs, "member_case_refs", nonempty=True, maximum=MAX_GROUP_MEMBERS, prefix="expanded_case_v2:"), reason_ref=exact_ref(reason_ref, "reason_ref"), review_refs=exact_review_refs(review_refs))
...
class PurposeContract:
    ...
    def create(cls, *, source_set_ref: str, memberships: tuple[PurposeMembership, ...], duplicate_risk_groups: tuple[DuplicateRiskGroup, ...], challenge_holdouts: tuple[ChallengeHoldout, ...], denominator_minima: tuple[DenominatorMinimum, ...], review_refs: tuple[str, ...], solver_output_is_authority: bool) -> "PurposeContract":
        ...
        case_refs = tuple(item.source_case_ref for item in memberships)
        if len(case_refs) != len(set(case_refs)):
            raise ValueError("duplicate source case membership")
        if case_refs != tuple(sorted(case_refs)):
        ...
        for rows, name, identity in (
            (duplicate_risk_groups, "duplicate-risk groups", lambda item: item.group_ref),
            (challenge_holdouts, "challenge holdouts", lambda item: item.holdout_ref),
        ):
        ...
        if minimum_identities != tuple(sorted(minimum_identities)):
            raise ValueError("denominator minima must be in canonical order")
        known_cases = set(case_refs)
        membership_by_case = {item.source_case_ref: item for item in memberships}
        group_refs = {item.group_ref for item in duplicate_risk_groups}
        for group in duplicate_risk_groups:
            ...
            if any(group.group_ref not in row.duplicate_risk_group_refs for row in member_rows):
        ...
        if any(set(row.duplicate_risk_group_refs) - group_refs for row in memberships):
        ...
        if {item.purpose for item in denominator_minima} != set(PURPOSES):
        ...
        material = {"abi_version": PURPOSE_CONTRACT_ABI_VERSION, "source_set_ref": exact_content_ref(source_set_ref, "source_set_ref", prefix="r4_source_set_v1:"), "memberships": [item.as_dict() for item in memberships], "duplicate_risk_groups": [item.as_dict() for item in duplicate_risk_groups], "challenge_holdouts": [item.as_dict() for item in challenge_holdouts], "denominator_minima": [item.as_dict() for item in denominator_minima], "review_refs": list(exact_review_refs(review_refs)), "solver_output_is_authority": solver_authority}
        return construct(cls, purpose_contract_ref=stable_ref("purpose_contract_v1", material), memberships=memberships, duplicate_risk_groups=duplicate_risk_groups, challenge_holdouts=challenge_holdouts, denominator_minima=denominator_minima, review_refs=tuple(material["review_refs"]), **{key: value for key, value in material.items() if key not in {"memberships", "duplicate_risk_groups", "challenge_holdouts", "denominator_minima", "review_refs"}})

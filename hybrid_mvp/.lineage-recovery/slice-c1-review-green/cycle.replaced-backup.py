...
class _PhaseMaterial:
    ...
    def __post_init__(self) -> None:
        ...
        if self.phase is SemanticPhase.EFFECT:
            ...
            if (
                self.disposition is PhaseDisposition.NO_EFFECT
                and target != source
            ):
                raise ValueError("NO_EFFECT cannot change a revision pin")
...
def _artifact_refs(
    *, orientation: Orientation | None, proposal: Any, verification: Any,
    evaluation: Any, effect_receipt: Any, response_meaning: Any,
    realization_receipt: Any, gap_receipt: GapReceipt | None,
) -> dict[str, str | None]:
...
class CycleFinalizer:
    ...
    def finalize(
        ...
        result = object.__new__(CycleResult)
        values = {
            "abi_version": CYCLE_RESULT_ABI_VERSION, "cycle_ref": cycle_ref,
            "input_ref": input_ref, "status": status,
            ...
            "realization_receipt": realization_receipt,
            "gap_receipt": gap_receipt, "phase_material": phase_material,
            "trace": trace, "final_revision_pin": final_revision_pin,
        }
...
# ---------------------------------------------------------------------------
# Temporary legacy typed-fixture compatibility (not canonical public ABI)
# ---------------------------------------------------------------------------


@dataclass(frozen=True)
class LegacyPhaseReceipt:
    cycle_ref: str
    phase: str
    input_refs: tuple[str, ...]
    output_refs: tuple[str, ...]
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            "cycle_ref": self.cycle_ref, "phase": self.phase,
            "input_refs": list(self.input_refs),
            "output_refs": list(self.output_refs),
            "revision_pin": self.revision_pin.as_dict(),
            "budget_use": dict(self.budget_use), "status": self.status,
            "rejection_codes": list(self.rejection_codes),
            "duration_ns": self.duration_ns,
        }
...
class KernelCycleResult:
    ...
    gap_receipt: GapReceipt | None
    trace: tuple[LegacyPhaseReceipt, ...]
    final_revision_pin: RevisionPin
    effect_receipt: "EffectReceipt | None" = None

    def as_dict(self) -> dict[str, Any]:
        return {
            "cycle_ref": self.cycle_ref, "status": self.status.value,
            "phase_output_refs": {
...
def _effect_receipt_as_dict(receipt: "EffectReceipt | None") -> dict[str, Any] | None:
    if receipt is None:
        return None
    return {
        "effect_ref": receipt.effect_ref, "status": receipt.status,

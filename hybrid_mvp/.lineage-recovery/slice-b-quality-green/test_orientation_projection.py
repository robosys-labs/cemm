...
def test_orientation_records_disabled_relation_adjacency_seam(runtime):
    ...
    assert not any(probe.startswith("by_kind:relation_type") for probe in orientation.index_probes)
...
class _ExplodingDependency:
    def __getattribute__(self, name: str):
        raise AssertionError(f"dependency used before projector input validation: {name}")
...
def test_projector_does_not_enumerate_unrelated_relation_types(runtime) -> None:
    ...
    assert orientation.index_probes.count("relation_adjacency:unavailable") == 1
    assert all(not probe.startswith("by_kind:relation_type") for probe in orientation.index_probes)
...
def test_projector_cache_key_is_exact_orientation_content_ref(runtime) -> None:
    ...
    mode = base_projector.project(
        "session:cache", "hello", mode=SemanticMode.QUERY
    )

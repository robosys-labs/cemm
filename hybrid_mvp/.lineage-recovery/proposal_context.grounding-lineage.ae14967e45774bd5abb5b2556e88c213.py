...
class ProposalContext:
    ...
    def create(
        ...
        *,
        ...
        return cls(context_ref=context_ref, **values)
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "ProposalContext":
        ...
        def rows(name: str, owner: Any) -> tuple[Any, ...]:
            value = data[name]
            if not isinstance(value, list):
                raise TypeError(f"{name} must be a list")
            return tuple(owner.from_dict(item) for item in value)

        raw_spans = data["source_unit_spans"]
        if not isinstance(raw_spans, list) or any(
            not isinstance(row, list) or len(row) != 3 for row in raw_spans
        ):
        ...
        rebuilt = cls.create(
            ...
            designation_slots=rows("designation_slots", DesignationSlot),
            contribution_slots=rows("contribution_slots", ContributionSlot),
            mode_slots=rows("mode_slots", ModeSlot),
            application_frames=rows("application_frames", ApplicationFrameSlot),
            reference_slots=rows("reference_slots", ReferenceSlot),
            scope_slots=rows("scope_slots", ScopeSlot),
            expression_link_slots=rows("expression_link_slots", ExpressionLinkSlot),
            variable_slots=rows("variable_slots", VariableSlot),
            transition_slots=rows("transition_slots", TransitionSlot),
            residual_evidence=rows("residual_evidence", ResidualEvidence),
            context_refs=_wire_string_tuple(data["context_refs"], "context_refs"),
            source_unit_refs=_wire_string_tuple(
                data["source_unit_refs"], "source_unit_refs"
            ),
    ...
    def build(
        ...
        *,
        ...
        return ProposalContext.create(
            orientation_ref=stable_ref("orientation", orientation),
...
def _validate_builder_inputs(
    ...
    if orientation.revision_pin is None:
        raise ValueError("orientation requires an exact revision pin")    if evidence.packet_ref != form_lattice.evidence_packet_ref:
    ...
    if grounding_result.revision_pin != orientation.revision_pin:
        raise ValueError("grounding revision pin disagrees with orientation revision pin")
    if evidence.form_pack_hash != form_lattice.form_pack_hash:
        raise ValueError("evidence and form lattice form pack hash disagree")
    scalar_pin = RevisionPin(
        orientation.authority_generation,
        orientation.world_revision,
        orientation.session_revision,
        orientation.episode_revision,
        orientation.effect_revision,
        orientation.model_identity,
    )
    if scalar_pin != orientation.revision_pin:
        raise ValueError("orientation scalar revisions disagree with revision pin")
    pinned_generation = orientation.revision_pin.authority_generation
    if getattr(authority, "generation", None) != pinned_generation:
        raise ValueError(
            "builder authority generation disagrees with orientation revision pin"
        )
    if getattr(affordance_index, "authority_generation", None) != pinned_generation:
        raise ValueError(
    ...
    if orientation.source_text and orientation.source_text != evidence.source_text:

...
| Activation Canary Receipt ABI | **1** | `src/cemm_authoritative_hybrid/cycle.py` | Serialized | `CycleFinalizer` | Persists content-bound refs for cycle, VerifiedMeaning, situation, Decision, effect/no-effect, ResponseMeaning, gap, final RevisionPin and canary row from real runtime cycles. |
...
| Corpus Review Manifest ABI | **1** | `src/cemm_authoritative_hybrid/r4_review.py` | Serialized external authorization | `ReviewManifestVerifier` | Binds exact source/artifact hashes and requires an externally supplied signature verifier; build code cannot self-approve. |
| Semantic Mutation ABI | **1** | `src/cemm_authoritative_hybrid/r4_mutations.py` | Serialized corpus candidate | `MutationExecutor` | One declared semantic/environment change with authentic earliest-owner observed label. |
| Partition Axis Manifest ABI | **1** | `src/cemm_authoritative_hybrid/r4_partitions.py` | Serialized data boundary | `IndependentAxisPartitioner` | Independently seals general, lexical, target, topology, dialogue, mutation and realization axes. |

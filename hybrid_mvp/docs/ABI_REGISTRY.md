...
| Corpus Review Manifest ABI | **2** | `src/cemm_authoritative_hybrid/r4_review.py` | Serialized external authorization | `ReviewManifestVerifier` | Binds exact source/artifact hashes, reviewer policy and external signature; plain callback verifiers and self-approval are forbidden. |
...
| R4 Build Receipt ABI | **2** | `src/cemm_authoritative_hybrid/r4_pipeline.py` | Serialized build evidence | `R4BuildReceipt.from_dict` + R4 admission verifier | Reconstructs every committed R4 artifact-set hash and remains `external_review_required`; it cannot self-approve. |
| Approved R4 Build ABI | **1** | `src/cemm_authoritative_hybrid/r4_pipeline.py` | Transient admission proof | `ApprovedR4Build.create` | Exists only after an externally pinned verifier authenticates Corpus Review Manifest ABI 2 and every manifest field matches the reconstructed Build Receipt ABI 2. |

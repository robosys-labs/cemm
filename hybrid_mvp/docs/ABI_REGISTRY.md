...
**Status:** active ABI contract for the CEMM Authoritative Hybrid MVP
**Runtime cutover: hard**

This registry records the owner file, serialized/transient status, validator
and activation gate for each active ABI. All active ABIs are at version 1.
...
| Phase Receipt ABI | 1 | `src/cemm_authoritative_hybrid/cycle.py` | Serialized (opt-in trace) | `KernelCycleResult` phase owner | Each of ORIENT, PROPOSE, VERIFY, EVALUATE, EFFECT, REALIZE emits one receipt with causal artifact refs and revision pins. |

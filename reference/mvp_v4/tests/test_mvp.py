...
from cemm_mvp import Store,Runtime,Inference,AmbiguousReferent,MODEL_CACHE,ExactStructuredCompiler  # noqa
...
class Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        MODEL_CACHE.clear();td,s,rt=make();rt.process('What is evidence?');s.db.close();td.cleanup()
    ...
    def test_trainer_deterministic_and_language_generic(self):
        ks=[BASE,FAMILY];a=compile_corpus(ROOT/'training/en_seed.json',ks);b=compile_corpus(ROOT/'training/en_seed.json',ks);self.assertEqual(a['pack_hash'],b['pack_hash']);self.assertTrue(a['structured_examples']);self.assertTrue(a['rule_examples'])

    def test_kernel_has_no_family_or_literal_response_hardcoding(self):
        src=((ROOT/'cemm_mvp.py').read_text()+(ROOT/'structured_codec.py').read_text()).lower()
    ...
    def test_reviewed_new_concept_acquisition_uses_same_codec(self):
        td,s,rt=make()
        try:
            doc=json.loads((ROOT/'acquisition_examples/friction.json').read_text());r=acquire(s,rt,doc);self.assertEqual(r['result']['status'],'learned');self.assertEqual(rt.process('What is Friction?')['response'],'Friction is resistance.');self.assertTrue(all(x.startswith('atom:') for x in r['created_or_resolved'].values()))
        finally:s.db.close();td.cleanup()
    ...
    def test_reviewed_designation_acquisition_changes_authority_but_user_fact_does_not(self):
        td,s,rt=make()
        try:
            h0=rt.runtime_attestation['authority_generation_hash'];rt.process('Ada is a doctor.');rt.reload_authority();self.assertEqual(h0,rt.runtime_attestation['authority_generation_hash']);acquire(s,rt,json.loads((ROOT/'acquisition_examples/friction.json').read_text()));self.assertNotEqual(h0,rt.runtime_attestation['authority_generation_hash'])
        finally:s.db.close();td.cleanup()

    def test_acquisition_does_not_add_operator_schema(self):
        td,s,rt=make()
        try:
            before=(s.db.execute("select count(*) from atoms where kind='operator'").fetchone()[0],s.db.execute('select count(*) from operator_roles').fetchone()[0]);acquire(s,rt,json.loads((ROOT/'acquisition_examples/friction.json').read_text()));after=(s.db.execute("select count(*) from atoms where kind='operator'").fetchone()[0],s.db.execute('select count(*) from operator_roles').fetchone()[0]);self.assertEqual(before,after)
        finally:s.db.close();td.cleanup()

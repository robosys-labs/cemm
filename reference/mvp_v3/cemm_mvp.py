...
class Store:
    ...
    def authority_hash(self,upto_generation=None):
        ...
        for t in ("atoms","rules"):
            cols=[x[1] for x in self.db.execute(f"PRAGMA table_info({t})") if x[1]!="generation"];rows=self.db.execute(f"SELECT {','.join(cols)} FROM {t} WHERE generation<=? ORDER BY {cols[0]}",(g,)).fetchall();material.append((t,[dict(r) for r in rows]))
        rows=self.db.execute("""SELECT a.app_ref,a.operator_ref,b.role_ref,b.filler_kind,b.filler_value,c.stance,c.authority_status
          ...
          WHERE a.generation<=? AND o.source_ref='seed' AND c.authority_status IN('reviewed','promoted') ORDER BY a.app_ref,b.role_ref,b.ordinal,c.stance""",(g,)).fetchall()
...
class NeuralCodecs:
    def __init__(self,pack:LanguagePack):
        self.pack=pack;ie=pack.data["interpretation_examples"];re=pack.data["realization_examples"];self.im,self.inet=train_classifier(ie,"delex_surface","program",19,130);self.im2,self.inet2=train_classifier(ie,"delex_surface","program",31,130);self.rm,self.rnet=train_classifier(re,"semantic","surface_plan",23,100);self.allowed={}
        for x in re:self.allowed.setdefault(norm_text(x["semantic"]),set()).add(norm_text(x["surface_plan"]))
    @classmethod
    def get(cls,pack):
        if pack.hash not in MODEL_CACHE:MODEL_CACHE[pack.hash]=cls(pack)
        return MODEL_CACHE[pack.hash]
    def interpret(self,delex):
        a,pa,ma,ka=predict_classifier(self.im,self.inet,delex);b,pb,mb,kb=predict_classifier(self.im2,self.inet2,delex);ok=a==b and min(ka,kb)>=.55 and min(ma,mb)>=.08
        return a,ok,{"delex":delex,"program":a,"independent_program":b,"confidence":min(pa,pb),"margin":min(ma,mb),"known_token_ratio":min(ka,kb),"independent_agreement":a==b,"verified":ok}
    def realize(self,semantic):
        plan,p,m,k=predict_classifier(self.rm,self.rnet,semantic);authorized=norm_text(plan) in self.allowed.get(norm_text(semantic),set());return plan,{"semantic":semantic,"surface_plan":plan,"confidence":p,"margin":m,"known_token_ratio":k,"authorized_transform":authorized}
...
class Interpreter:
    def __init__(self,s,pack):self.s,self.pack=s,pack;self.lang=pack.language
    def parse(self,text):
        delex,ph,uses=Delexer(self.s,self.lang).run(text);program,ok,trace=NeuralCodecs.get(self.pack).interpret(delex)
        if not ok:return None,[],uses,{**trace,"reason":"semantic_program_unsettled"}
...
class Inference:
    ...
    def closure(self,extra=()):
        self.incomplete=False;self.incomplete_reason=None;facts=list(self.s.base_facts())+list(extra);bysig={f.signature():f for f in facts};byref={f.ref:f for f in facts};rules=[dict(r) for r in self.s.db.execute("SELECT * FROM rules WHERE rule_kind IN('definition','entailment') AND authority_status IN('reviewed','promoted') ORDER BY rule_ref")]
...
class PointerRealizer:
    def __init__(self,s,pack):self.s,self.pack=s,pack;self.codec=NeuralCodecs.get(pack)
...
class Runtime:
    def __init__(self,s,pack_path,top_k=24):
        self.s=s;self.pack=LanguagePack(pack_path);self.lang=self.pack.language;self.i=Interpreter(s,self.pack);self.inf=Inference(s);self.selfstate=SessionSelf(s);self.ws=Workspace(s,self.selfstate,top_k);self.r=PointerRealizer(s,self.pack);self.planner=ResponsePlanner(s);self.runtime_attestation={"authority_generation":s.generation,"authority_generation_hash":s.authority_hash(s.generation),"language_pack_hash":self.pack.hash,"read_generation":s.generation}
...
def main():
    p=argparse.ArgumentParser();p.add_argument("command",choices=["init","chat","learn","ask","inspect"]);p.add_argument("text",nargs="?");p.add_argument("--db",default="cemm_mvp.sqlite");p.add_argument("--data",action="append",default=[]);p.add_argument("--pack",required=True);a=p.parse_args();s=Store(a.db)
